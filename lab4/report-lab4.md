...
Цель работы
Спроектировать инфраструктуру AI-приложения для трёх стадий жизненного цикла, рассчитать экономическую модель и обосновать выбор облачных ресурсов.
Описание приложения
Приложение: AI-трекер питания и тренировок «PlateCoach». Пользователь фотографирует блюдо, приложение распознаёт продукты, считает КБЖУ, связывает приёмы пищи с тренировками и выдаёт персональные рекомендации.
AI-компонент: распознавание блюд по фото и генерация рекомендаций через LLM (Vision-модели). На стадиях 1-2 используется внешний API (OpenRouter), на стадии 3 добавляется собственная локальная LLM в кластере.
Почему OpenRouter: единая точка входа к разным моделям (GPT, Claude, Gemini, Llama), переключение между провайдерами без изменения кода, прозрачный биллинг.
Функциональные модули:
МодульFrontendBackendСтартовая страница, регистрациядадаЛичный кабинет, целидадаЛента приёмов пищидадаЗагрузка фото и распознавание блюддадаЖурнал тренировокдадаРекомендации питаниянетдаПлатежи (подписка)дадаПоддержка пользователейдадаУведомлениянетдаAdmin panel и BIдадаМониторинг и алертингнетда
Ожидаемые нагрузки:
