University: ITMO University (https://itmo.ru/ru/) Faculty: FTMI

Course: Cloud Platforms as the basis of technology entrepreneurship Year: 2025/2026

Group: U4125

Author: Vasilev Anton

Lab: Lab1

Date of create: 05.05.2026

# Лабораторная работа №1

# Обзор Google Cloud и исследование основных сервисов

- Создал VM с нужными данными e2-micro в режиме spot. Выбрал правильный нейминг итд.

<img width="1962" height="132" alt="Screenshot_1" src="https://github.com/user-attachments/assets/781afc30-0b49-4970-8c66-539fa62186d7" />

- Проверил что ВМ запущена и корректно все отображается. 

<img width="1353" height="1172" alt="Screenshot_2" src="https://github.com/user-attachments/assets/811621cd-880a-4c05-a8d8-a40c300dfbfd" />
- Подключился по SSH к консоли. Нашел бакет с данными (картинками) и скопировал его в директорию на своей VM.

<img width="1347" height="234" alt="Screenshot_3" src="https://github.com/user-attachments/assets/6e182ec2-23a5-4a72-a869-7647ff3918f1" />

<img width="903" height="701" alt="Screenshot_4" src="https://github.com/user-attachments/assets/b07b7a1f-1f2d-4908-9b01-eaccd8c2d79b" />

- Изменил права на Compute Viewer. После смены роли с Storage Admin на Compute Viewer service account теряет часть прав.
<img width="586" height="536" alt="Screenshot_5" src="https://github.com/user-attachments/assets/d5c3966f-d722-4b23-a7de-c5ee20240974" />

- Удалил все созданные сервисы.
<img width="617" height="363" alt="Screenshot_6" src="https://github.com/user-attachments/assets/6c3625a5-e81c-4314-9a5f-6d26646c561a" />



